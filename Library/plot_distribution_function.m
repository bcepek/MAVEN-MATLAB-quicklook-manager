% timestart = datenum('04-Jan-2015 20:50:00');
% timeend = datenum('04-Jan-2015 20:55:00');

function h_plot = plot_distribution_function(timestart, timeend, mass_num, epoch, eflux, energy, swp_ind, magf, quat_mso, energies_to_plot)
% mass_num = 1;
...
while(start_time+timelen-1 <= stop_time)
    ...
    %eflux_disp(:, :, :, :, 1) = eflux(:, :, :, tft(1):tft(2));
    %verify = eflux_disp ~=0;
    %colordata = [min(min(min(min(min(log10(eflux_disp(verify))))))), max(max(max(max(max(log10(eflux_disp(verify)))))))];
    ...
    for enum = energies_to_plot
        for timenum =1:timelen
            ...
            xlim([min(dotcont(:, 1)) max(dotcont(:, 1))])
            axis equal
            hold off
