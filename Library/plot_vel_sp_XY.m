function xticks = plot_vel_sp_XY(TimeStart,mass_num,x,y,z,epoch,eflux,mass_arr,energy,swp_ind,quat_mso,phi,theta,mag_filename)
...
for time_ind_add = 0:2
    ...
    scatter3(vec_mso(ion_mark,1)+double(repmat(sc_vel1(1),length(vec_mso(ion_mark,1)),1)),...
        vec_mso(ion_mark,2)+double(repmat(sc_vel1(2),length(vec_mso(ion_mark,2)),1)),...
        vec_mso(ion_mark,3)+double(repmat(sc_vel1(3),length(vec_mso(ion_mark,3)),1)),...
    ...
    B = B/sqrt(sum(B.^2,2));
    max_len = max(sqrt(sum(vec_mso.^2,2)));
    B = B*max_len/2;
