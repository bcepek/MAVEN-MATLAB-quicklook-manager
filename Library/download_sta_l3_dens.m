function filename = download_sta_l3_dens(time)

server = "berkeley";
%server = "lasp";
if server == "berkeley"
    server_url = 'https://sprg.ssl.berkeley.edu/data/maven/data/sci/sta/l3/density/';
elseif server == "lasp"
...
download_path = [paths.sta_l3 slash mvn_year slash mvn_month slash];
...
try
    ...
    filename = -1;
    disp('sta l3 download unsuccessful')
    return
end

pattern = ['mvn_sta_l3_den_' mvn_year mvn_month mvn_day '_v'] + digitsPattern(2) + '_r' + digitsPattern(2) + '.cdf';
...
fname = web_page(fname_ind(1):fname_ind(1)+34);
